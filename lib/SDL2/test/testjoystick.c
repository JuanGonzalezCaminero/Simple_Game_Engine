...
/* Simple program to test the SDL2 joystick routines */
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_KEYDOWN:
            /* Press the L key to lag for 3 seconds, to see what happens
                when SDL2 doesn't service the event loop quickly. */
...
main(int argc, char *argv[])
{
    ...
    /* Initialize SDL2 (Note: video is required to start event loop) */
...
main(int argc, char *argv[])
{
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "SDL2 compiled without Joystick support.\n");
    return 1;
}
