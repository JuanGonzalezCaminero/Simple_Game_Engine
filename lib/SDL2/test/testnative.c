/*
  ...
  freely.
*/
/* Simple program:  Create a native window and attach an SDL2 renderer */
...
/* Call this instead of exit(), so we can clean up SDL2: atexit() is evil. */
static void
quit(int rc)
{
    SDL_VideoQuit();
...
main(int argc, char *argv[])
{
    ...
    if (SDL_VideoInit(NULL) < 0) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Couldn't initialize SDL2 video: %s\n",
                SDL_GetError());
        exit(1);
    }
